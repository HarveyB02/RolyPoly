        ...
        if (args.length > 1) { // Timed mute (uses database)
            ...
            switch (unit) {
                ...
                default: 
                    ...
                    var description = `Invalid time unit was provided, please use m, d or w \`\`\`\n${data.command.dmOnly ? '' : message.guild.prefix}${data.command.name} ${data.command.arguments.replace(/~.+?(?=( |$))/g, '')}\`\`\`For example:\`\`\`${message.guild.prefix}mute @Harvey 30m\`\`\``;
            ...
            if (isNaN( parseInt(duration) )) {
                // Error msg
                var description = `Invalid number was provided, please use \`\`\`\n${data.command.dmOnly ? '' : message.guild.prefix}${data.command.name} ${data.command.arguments.replace(/~.+?(?=( |$))/g, '')}\`\`\``;
