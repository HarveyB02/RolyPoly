...
module.exports = {
    ...
    execute: async ({ client, message, args }) => {
        ...
        member.send('Hey ' + member.user.username + ', ' + message.author.username + ' from ' + member.guild.name + ' asked me to give you some info! I\'m ' + message.guild.name + '\'s subject manager, my purpose is to help you navigate the server.\n\nHere\'s a quick rundown of my features and how to use them.\n_ _', {embed: embed1})
            .then(() => {
                member.send(embed2);
                message.react('✅');
            })
            .catch(() => {
                console.log(`Could not send welcome message to ${member.user.tag}`);
                message.reply(`Could not send welcome message to ${member.user.tag}`);
            });
    }
}